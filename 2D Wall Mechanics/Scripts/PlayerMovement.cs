...
public class PlayerMovement : MonoBehaviour
{
    ...
    [SerializeField] public float initialGravity;
    ...
    public WallMechanics wallMechanics;
    ...
    public void Movement()
    {
        ...
        if (playerGrounded)
        {
            ...
            if ((inputManager.GetHorizontalInput() > 0.0f && !isFacingRight) ||
                            inputManager.GetHorizontalInput() < 0.0f && isFacingRight && !wallMechanics.CanWallJump)
            {
                FlipCharacter();
            }
        }
        ...
        else
        {
            ...
            // If a wall is detected or player currently performing a wall jump then character shouldn't be flipped.
            // In the case of wall jumping, character flipping determined differnetly in wall jumping method.
            if (!wallMechanics.WallCheck() && !wallMechanics.CanWallJump)
            {
    ...
    private void HandlePlayerAnimations()
    {
        ...
        playerAnimator.SetBool("WallSlide", wallMechanics.IsWallSliding);

        if (playerGrounded)
        {
            playerAnimator.SetBool("Move", Mathf.Abs(inputManager.GetHorizontalInput()) > 0.05f & !wallMechanics.WallCheck());
    ...
    private void HandleDifferentJumpMechanics()
    {
        ...
        if (!playerGrounded && !wallMechanics.IsWallClimbing)
        {
